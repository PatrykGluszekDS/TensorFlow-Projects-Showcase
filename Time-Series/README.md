...
* **Model 6 (Dense multivariate – rentals + temperature):** Adding a single weather feature (`temp`) and training a multivariate Dense model brings performance roughly back to the level of the better univariate Dense models (MAE ≈ 804, MASE ≈ 0.92). With only one extra covariate and a simple architecture, the gains are modest but it demonstrates how to incorporate exogenous features.
* **Model 7 (N-BEATS):** Implementing a compact N-BEATS-style architecture on top of the same 7-day window produces one of the strongest univariate models (MASE ≈ 0.89, on par with the best Dense-7). This experiment shows how ideas from more recent forecasting research can be adapted to a small practical problem.
